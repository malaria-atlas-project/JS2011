...
c subsequent dimensions iterate over spatial dimensions.
      ...
      SUBROUTINE exp_cov(C,nx,ny,symm)
      ...
      SUBROUTINE matern(C,nu,nx,ny,symm,BK,N)

cf2py double precision dimension(nx,ny),intent(inplace)::C
cf2py double precision intent(in),check(nu>0)::nu
